...
from .models import Cart, CartItem, Coupon, ShippingCoupon
...
class ShippingCouponSerializer(serializers.ModelSerializer):
    class Meta:
        model = ShippingCoupon
...
class CartSerializer(serializers.ModelSerializer):
    ...
    shipping_coupon = ShippingCouponSerializer(read_only=True)
    ...
    shipping_discount = serializers.DecimalField(max_digits=12, decimal_places=2, read_only=True)
...
class ApplyShippingCouponSerializer(serializers.Serializer):
    coupon_code = serializers.CharField(max_length=50, required=True)

    def validate_coupon_code(self, value):
        try:
            coupon = ShippingCoupon.objects.get(code=value, active=True)
        except ShippingCoupon.DoesNotExist:
            raise serializers.ValidationError(
                "Invalid or inactive shipping coupon code"
            )
        return coupon
